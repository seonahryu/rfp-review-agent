...
assert(apiMain.includes("manual_compliance_content: str ="), "backend feedback model should accept manually entered recommendation content")
assert(apiMain.includes("apply_manual_compliance_contents"), "backend should apply manual recommendation content after generation")
...
for (const uiRoot of uiRoots) {
  ...
  assert(finalStep.includes("전체 명시 여부 복사"), "final step should show row-level internal assessment copy for items 5 and 6")
  assert(finalStep.includes("renderDetailedAssessment"), "final recommendation cell should render internal assessment tables")
  assert(finalStep.includes("internalAssessmentCopyText"), "final step should build one-shot copy text for internal assessment tables")
  assert(finalStep.includes("item.detailed_assessment"), "final step should use detailed assessment data when available")
  ...
  assert(types.includes("manual_compliance_content?: string"), "feedback type should include manual recommendation content")
  assert(resultItemCard.includes("법령준수여부 수정"), "result confirmation card should let users correct legal compliance result")
  assert(resultItemCard.includes("권고내용 직접 입력"), "result confirmation card should let users enter final recommendation content")
  assert(resultItemCard.includes("manual_compliance_content: manualContent.trim()"), "manual recommendation content should be submitted")
  assert(resultItemCard.includes("준수: 제안요청서 p._ 명시"), "manual recommendation guide should show compliant example")
  assert(resultItemCard.includes("해당없음: 해당없음"), "manual recommendation guide should show not-applicable example")
  assert(resultItemCard.includes('option.value === "해당없음"'), "not-applicable selection should autofill manual recommendation content")
